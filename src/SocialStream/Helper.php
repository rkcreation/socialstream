...
EOD;
    ...
    public static function truncate($string, $max_length, $wordsafe = false, $add_ellipsis = false, $min_wordsafe_length = 1) {
        ...
        if ($wordsafe) {
            ...
            $found = preg_match('/^(.{' . $min_wordsafe_length . ',' . $max_length . '})[' . self::PREG_CLASS_WORD_BOUNDARY . ']/us', $string, $matches);
            if ($found) {
                $string = $matches[1];
            }
            else {
    ...
    /**
     * @param $url
     * @param bool $ouputJson
     *
     * @return mixed
     */
    public static function curl($url, $ouputJson = true) {
